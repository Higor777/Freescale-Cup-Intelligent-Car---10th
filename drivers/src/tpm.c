...
static void _TPM_InitBasic(uint32_t instance, uint32_t modulo, TPM_ClockDiv_Type ps)
{
    ...
    *(uint32_t*)SIM_TPMClockGateTable[instance].addr |= SIM_TPMClockGateTable[instance].mask;
    ...
    TPM_InstanceTable[instance]->CNT = 0;
	  //TPM_InstanceTable[instance]->CNTIN = 0;
 ...
 * @retval None
 */
void TPM_IC_QuickInit(uint32_t MAP, TPM_ClockDiv_Type ps)
{
    uint32_t i;
    QuickInit_Type * pq = (QuickInit_Type*)&(MAP);
    /* init pinmux */
    ...
    _TPM_InitBasic(pq->ip_instance, TPM_MOD_MOD_MASK, ps);
    TPM_SetMode(pq->ip_instance, pq->channel, kTPM_Mode_InputCapture);
}
 ...
 * @retval None
 */
void TPM_IC_SetTriggerMode(uint32_t instance, uint32_t chl, TPM_IC_Mode_Type mode)
{
    /* clear ELSB & ELSA */
    TPM_InstanceTable[instance]->CONTROLS[chl].CnSC &= ~(TPM_CnSC_ELSB_MASK|TPM_CnSC_ELSA_MASK);
    switch(mode)
    {
        case kTPM_IC_FallingEdge:
            TPM_InstanceTable[instance]->CONTROLS[chl].CnSC &= ~(TPM_CnSC_ELSB_MASK);
            TPM_InstanceTable[instance]->CONTROLS[chl].CnSC |= (TPM_CnSC_ELSA_MASK);
            break;
        case kTPM_IC_RisingEdge:
            TPM_InstanceTable[instance]->CONTROLS[chl].CnSC |= (TPM_CnSC_ELSB_MASK);
            TPM_InstanceTable[instance]->CONTROLS[chl].CnSC &= ~(TPM_CnSC_ELSA_MASK);
            break;
        case kTPM_IC_RisingFallingEdge:
            TPM_InstanceTable[instance]->CONTROLS[chl].CnSC |= (TPM_CnSC_ELSB_MASK|TPM_CnSC_ELSA_MASK);
            break;
        default:
            break;
...
TPM_Type TPMN[3] = {TPM0_BASE, TPM1_BASE, TPM2_BASE};

void tpm_pulse_init(TPMn_e tpmn,TPM_CLKIN_e clkin,TPM_ClockDiv_Type ps)
{
    switch(tpmn)
    {
    case HW_TPM0:
    ...
    if(clkin == TPM_CLKIN0)
    {
        SIM->SOPT4 &= ~( SIM_SOPT4_TPM0CLKSEL_MASK << tpmn );
        PORT_PinMuxConfig(HW_GPIOE,29,kPinAlt4);
    }
    else if(clkin == TPM_CLKIN1)
    {
        SIM->SOPT4 |= ( SIM_SOPT4_TPM0CLKSEL_MASK << tpmn );
        PORT_PinMuxConfig(HW_GPIOB,17,kPinAlt4);
    }
    else
    {
        ;
    ...
    SIM->SOPT2 &= ~ SIM_SOPT2_TPMSRC_MASK;
    SIM->SOPT2 |= SIM_SOPT2_TPMSRC(1);        //     00 Clock disabled  01 MCGFLLCLK clock, or MCGPLLCLK/2  10 OSCERCLK clock   11 MCGIRCLK clock

    TPM_InstanceTable[tpmn]->SC    = ( 0
                                  //| TPM_SC_CPWMS_MASK
                                  | TPM_SC_PS(ps)
                                  | TPM_SC_CMOD(2)
                               );
    TPM_InstanceTable[tpmn]->MOD   = 0xFFFF;
    TPM_InstanceTable[tpmn]->CNT  = 0;
}
...
{
    return (uint16_t)TPM_InstanceTable[tpmn]->CNT;
}
